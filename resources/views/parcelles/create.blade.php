...
<div class="card" style="max-width:800px"><div class="card-body">
    <form method="POST" action="{{ isset($parcelle) ? route('parcelles.update', $parcelle) : route('parcelles.store') }}">
        ...
        <div class="form-row">
            <div class="form-group"><label class="form-label">Type employés</label><select name="type_employes" class="form-select"><option value="">—</option>@foreach(['SEASONAL'=>'Saisonniers','PERMANENT'=>'Permanents','FAMILIAL_LABOR'=>'Familiale main d\'œuvre'] as $k=>$l)<option value="{{ $k }}" {{ old('type_employes', $parcelle->type_employes ?? '') === $k ? 'selected' : '' }}>{{ $l }}</option>@endforeach</select></div>
